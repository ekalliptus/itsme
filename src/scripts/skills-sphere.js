...
export const THEME_COLORS = {
  dark: {
    background: 0x101b20,      // Dark background (transparent in actual use)
    nodeColor: 0xffffff,       // Light/white icons for dark mode
    textColor: 0xffffff,       // Light text for dark mode
    glowColor: 0x88d4c5,       // Mint accent glow
    hoverGlow: 0x88d4c5        // Hover glow color
  },
  light: {
    background: 0x101b20,      // Dark background (transparent in actual use)
...
export class SkillsSphere {
  ...
  async initThree() {
    ...
    // Navigation or the visualization toggle can dispose the scene while loading.
    ...
    // Start animation loop (reduced motion renders one static frame instead)
   ...
   */
  resume() {
    if (this.isPaused && this.isInitialized && !this.reducedMotion) {
      this.isPaused = false;
      this.animate();
    }
  }
